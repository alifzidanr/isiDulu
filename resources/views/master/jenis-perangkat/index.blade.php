<!-- resources/views/master/jenis-perangkat/index.blade.php -->
@extends('layouts.app')

@section('title', 'Master Jenis Perangkat - HelpDesk')

@section('content')
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Master Jenis Perangkat</h1>
        <button onclick="openCreateModal()" 
            ...
            <i class="fas fa-plus mr-2"></i>Tambah Jenis Perangkat
        </button>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($jenisPerangkats as $index => $jenisPerangkat)
                    <tr>
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                            {{ $jenisPerangkats->firstItem() + $index }}
                        </td>
                        ...
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                            <div class="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
                                <button onclick="openEditModal({{ $jenisPerangkat->id_jenis_perangkat }}, '{{ $jenisPerangkat->nama_perangkat }}')" 
                                        class="text-blue-600 hover:text-blue-900 text-left">
                                    <i class="fas fa-edit mr-1"></i>Edit
                                </button>
                                <form id="deleteForm{{ $jenisPerangkat->id_jenis_perangkat }}" action="{{ route('master.jenis-perangkat.destroy', $jenisPerangkat->id_jenis_perangkat) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                                            onclick="confirmDelete({{ $jenisPerangkat->id_jenis_perangkat }}, '{{ $jenisPerangkat->nama_perangkat }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-4 sm:px-6 py-4 text-center text-gray-500">Tidak ada data jenis perangkat</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Tambah Jenis Perangkat</h3>
            <button onclick="closeCreateModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        
        <form action="{{ route('master.jenis-perangkat.store') }}" method="POST" class="space-y-3 sm:space-y-4">
            @csrf
            <div>
                <label for="nama_perangkat" class="block text-sm/6 font-medium text-gray-900">Nama Perangkat</label>
                <div class="mt-2">
                    <input type="text" name="nama_perangkat" id="nama_perangkat" required
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Edit Jenis Perangkat</h3>
        ...
        <form id="editForm" method="POST" class="space-y-3 sm:space-y-4">
            ...
            @method('PUT')
            <div>
                <label for="edit_nama_perangkat" class="block text-sm/6 font-medium text-gray-900">Nama Perangkat</label>
                <div class="mt-2">
        ...
        <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
            <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    <h3 class="text-base font-semibold text-gray-900">Hapus Jenis Perangkat</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500" id="deleteConfirmMessage">Apakah Anda yakin ingin menghapus jenis perangkat ini? Semua data yang terkait akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
            </div>
...
function openEditModal(id, nama) {
    ...
    document.body.classList.add('overflow-hidden');
    document.getElementById('editForm').action = `/master/jenis-perangkat/${id}`;
...
function confirmDelete(id, nama) {
    ...
    document.getElementById('deleteConfirmMessage').textContent = 
        `Apakah Anda yakin ingin menghapus jenis perangkat "${nama}"? Semua data yang terkait akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.`;

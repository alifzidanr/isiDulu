...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Master Jenis Perawatan</h1>
        <button onclick="openCreateModal()" 
            ...
            <i class="fas fa-plus mr-2"></i>Tambah Jenis Perawatan
        </button>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-4 sm:px-6 py-4 text-center text-gray-500">Tidak ada data jenis perawatan</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Tambah Jenis Perawatan</h3>
            <button onclick="closeCreateModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        
        <form action="{{ route('master.jenis-perawatan.store') }}" method="POST" class="space-y-3 sm:space-y-4">
            @csrf
            <div>
                <label for="nama_perawatan" class="block text-sm/6 font-medium text-gray-900">Nama Perawatan</label>
                <div class="mt-2">
                    <input type="text" name="nama_perawatan" id="nama_perawatan" required
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Edit Jenis Perawatan</h3>
        ...
        <form id="editForm" method="POST" class="space-y-3 sm:space-y-4">
            ...
            @method('PUT')
            <div>
                <label for="edit_nama_perawatan" class="block text-sm/6 font-medium text-gray-900">Nama Perawatan</label>
                <div class="mt-2">
                    <input type="text" name="nama_perawatan" id="edit_nama_perawatan" required
                        placeholder="Masukkan nama perawatan"
                        class="block w-full rounded-md bg-white px-3 py-2 sm:py-1.5 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-blue-600 sm:text-sm/6">
                </div>
            </div>
...
function closeCreateModal() {
    ...
    document.getElementById('nama_perawatan').value = '';
}

function openEditModal(id, nama) {
    document.getElementById('editModal').classList.remove('hidden');
    document.body.classList.add('overflow-hidden');
    document.getElementById('editForm').action = `/master/jenis-perawatan/${id}`;
    document.getElementById('edit_nama_perawatan').value = nama;
}

<!-- resources/views/master/detail-perawatan/index.blade.php -->
@extends('layouts.app')

@section('title', 'Master Detail Perawatan - HelpDesk')

@section('content')
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Master Detail Perawatan</h1>
        <button onclick="openCreateModal()" 
            ...
            <i class="fas fa-plus mr-2"></i>Tambah Detail Perawatan
        </button>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Detail Perawatan</th>
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($detailPerawatans as $index => $detailPerawatan)
                    <tr>
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                            {{ $detailPerawatans->firstItem() + $index }}
                        </td>
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-900">
                            {{ $detailPerawatan->nama_detail_perawatan }}
                        </td>
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                            <div class="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
                                <button onclick="openEditModal({{ $detailPerawatan->id_detail_perawatan }}, '{{ $detailPerawatan->nama_detail_perawatan }}')" 
                                        class="text-blue-600 hover:text-blue-900 text-left">
                                    <i class="fas fa-edit mr-1"></i>Edit
                                </button>
                                <form id="deleteForm{{ $detailPerawatan->id_detail_perawatan }}" action="{{ route('master.detail-perawatan.destroy', $detailPerawatan->id_detail_perawatan) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                                            onclick="confirmDelete({{ $detailPerawatan->id_detail_perawatan }}, '{{ $detailPerawatan->nama_detail_perawatan }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-4 sm:px-6 py-4 text-center text-gray-500">Tidak ada data detail perawatan</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Tambah Detail Perawatan</h3>
            <button onclick="closeCreateModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        
        <form action="{{ route('master.detail-perawatan.store') }}" method="POST" class="space-y-3 sm:space-y-4">
            @csrf
            <div>
                <label for="nama_detail_perawatan" class="block text-sm/6 font-medium text-gray-900">Nama Detail Perawatan</label>
                <div class="mt-2">
                    <input type="text" name="nama_detail_perawatan" id="nama_detail_perawatan" required
                        placeholder="Masukkan nama detail perawatan"
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-11/12 sm:w-96 shadow-lg rounded-md bg-white max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center pb-3">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Edit Detail Perawatan</h3>
        ...
        <form id="editForm" method="POST" class="space-y-3 sm:space-y-4">
            ...
            @method('PUT')
            <div>
                <label for="edit_nama_detail_perawatan" class="block text-sm/6 font-medium text-gray-900">Nama Detail Perawatan</label>
                <div class="mt-2">
                    <input type="text" name="nama_detail_perawatan" id="edit_nama_detail_perawatan" required
                        placeholder="Masukkan nama detail perawatan"
                        class="block w-full rounded-md bg-white px-3 py-2 sm:py-1.5 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-blue-600 sm:text-sm/6">
                </div>
            </div>
        ...
        <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
            <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    <h3 class="text-base font-semibold text-gray-900">Hapus Detail Perawatan</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500" id="deleteConfirmMessage">Apakah Anda yakin ingin menghapus detail perawatan ini? Semua data yang terkait akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
            </div>
...
function closeCreateModal() {
    ...
    document.getElementById('nama_detail_perawatan').value = '';
}

function openEditModal(id, nama) {
    document.getElementById('editModal').classList.remove('hidden');
    document.body.classList.add('overflow-hidden');
    document.getElementById('editForm').action = `/master/detail-perawatan/${id}`;
    document.getElementById('edit_nama_detail_perawatan').value = nama;
}
...
function confirmDelete(id, nama) {
    ...
    document.getElementById('deleteConfirmMessage').textContent = 
        `Apakah Anda yakin ingin menghapus detail perawatan "${nama}"? Semua data yang terkait akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.`;
